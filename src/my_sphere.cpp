 ...
 * - flip_normals
   - |bool|
   - Is the sphere inverted, i.e. should the normal vectors be flipped?
 ...
 * - silhouette_sampling_weight
   - |float|
   - Weight associated with this shape when sampling silhoeuttes in the scene.
(Default: 1)
   - |exposed|
...
.. subfigure::
../../resources/data/docs/images/render/shape_sphere_parameterization.jpg
...
A sphere can either be configured using a linear :monosp:`to_world`
...
.. tabs::
    ...
    .. code-tab:: python
        ...
        'sphere_1': {
            'type': 'sphere',
            'to_world': mi.ScalarAffineTransform4f().scale([2, 2,
2]).translate([1, 0, 0]), 'bsdf': { 'type': 'diffuse'
            }
        },
...
When a :ref:`sphere <shape-sphere>` shape is turned into an :ref:`area
<emitter-area>` light source, Mitsuba 3 switches to an efficient `sampling
strategy <https://www.akalin.com/sampling-visible-sphere>`_ by Fred Akalin that
has particularly low variance.
This makes it a good default choice for lighting new scenes.

.. subfigstart::
.. subfigure::
../../resources/data/docs/images/render/shape_sphere_light_mesh.jpg :caption:
Spherical area light modeled using triangles
.. subfigure::
../../resources/data/docs/images/render/shape_sphere_light_analytic.jpg
   :caption: Spherical area light modeled using the :ref:`sphere <shape-sphere>`
plugin
.. subfigend::
...
class MySphere final : public Shape<Float, Spectrum> {
public:
  ...
  MySphere(const Properties &props) : Base(props) {
  ...
  SilhouetteSample3f sample_silhouette(const Point3f &sample, uint32_t flags,
    ...
    SilhouetteSample3f ss(sample_position(0.f, dr::tail<2>(sample), active));
  ...
  SilhouetteSample3f primitive_silhouette_projection(
      const Point3f &viewpoint, const SurfaceInteraction3f &si, uint32_t flags,
    ...
    Vector3f OXd = dr::normalize(
        sin_phi * OVd + dr::safe_sqrt(-dr::fmsub(sin_phi, sin_phi, 1.f)) * OYd);
  ...
  void optix_prepare_geometry() override {
    if constexpr (dr::is_cuda_v<Float>) {
      if (!m_optix_data_ptr)
        m_optix_data_ptr =
            jit_malloc(AllocType::Device, sizeof(OptixSphereData));

      OptixSphereData data = {bbox(), (Vector<float, 3>)m_center.scalar(),
